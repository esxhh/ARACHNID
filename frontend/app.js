...
async function loadData() {
  try {
    ...
    document.querySelector("#service-status").textContent = "Data plane unavailable \u2013 retrying";
...
function renderVessels() {
  ...
  state.vessels.forEach((vessel, index) => {
    const marker = L.marker([vessel.lat, vessel.lon], { icon: vesselIcon }).on("click", () => selectVessel(vessel.mmsi)).bindTooltip(`<strong>${escapeHtml(vessel.name)}</strong><br>${escapeHtml(vessel.type)} / ${escapeHtml(vessel.speed)} kn`, { direction: "top", offset: [0, -6] });
...
function renderFleet() {
  ...
  state.vessels.forEach((vessel) => {
    ...
    item.innerHTML = `<div class="incident-card-top"><strong>${escapeHtml(vessel.name)}</strong><span class="incident-confidence">${escapeHtml(vessel.speed)} kn</span></div><div class="incident-card-bottom"><small>${escapeHtml(vessel.type)} / ${escapeHtml(vessel.flag)}</small><small><i class="status-mini"></i>${escapeHtml(vessel.status)}</small></div>`;
...
function renderSearchResults(query) {
  ...
  results.innerHTML = matches.map((vessel) => `<button data-mmsi="${escapeHtml(vessel.mmsi)}"><strong>${escapeHtml(vessel.name)}</strong><small>${escapeHtml(vessel.mmsi)} / ${escapeHtml(vessel.flag)}</small></button>`).join("");
...
function renderDetail(vessel) {
  document.querySelector("#detail-content").innerHTML = `<div class="vessel-hero"><span class="hero-vessel-icon">+</span><div><strong>${escapeHtml(vessel.name)}</strong><small>${escapeHtml(vessel.type)} / ${escapeHtml(vessel.flag)} flagged</small></div><span class="live-chip"><i></i> LIVE</span></div><div class="detail-summary"><div class="summary-cell"><span>MMSI</span><strong>${escapeHtml(vessel.mmsi)}</strong></div><div class="summary-cell"><span>STATUS</span><strong>${escapeHtml(vessel.status)}</strong></div><div class="summary-cell"><span>SPEED</span><strong>${escapeHtml(vessel.speed)} kn</strong></div><div class="summary-cell"><span>COURSE</span><strong>${escapeHtml(vessel.course)} deg</strong></div></div><div class="rank-heading"><span>Current position</span><small>${escapeHtml(vessel.position_source || "AIS")}</small></div><div class="position-readout"><span>LAT ${vessel.lat.toFixed(4)} N</span><span>LON ${vessel.lon.toFixed(4)} E</span></div><div class="rank-heading"><span>Route signal</span><small>heading ${escapeHtml(vessel.course)} deg</small></div><div class="motion-bar"><span style="width: ${Math.min(100, Math.max(12, vessel.speed * 5))}%"></span></div><p class="motion-note">This vessel is moving at ${escapeHtml(vessel.speed)} knots. ARACHNID can explain AIS, routes, vessel types, and maritime context.</p>`;
}
