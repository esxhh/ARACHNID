...
def _clean_text(value, max_length: int = MAX_TEXT_FIELD_LENGTH) -> str | None:
    """Coerce to a plain, length-capped string so ingested fields can never
    carry HTML/script payloads into the frontend's innerHTML rendering."""
...
def _clean_fix(vessel: dict) -> dict | None:
    """Validate one AIS fix, rejecting anything with missing/out-of-range fields."""
    required = {"mmsi", "name", "lat", "lon", "speed", "course", "status"}
    if not isinstance(vessel, dict) or not required.issubset(vessel):
        return None
    mmsi = _clean_text(vessel.get("mmsi"), max_length=9)
    name = _clean_text(vessel.get("name"))
    vessel_type = _clean_text(vessel.get("type")) or "Unknown"
    flag = _clean_text(vessel.get("flag"), max_length=3) or "--"
    status = _clean_text(vessel.get("status")) or "Underway"
    try:
    ...
    if not (-90 <= lat <= 90) or not (-180 <= lon <= 180):
        return None
    if not (0 <= speed <= 60) or not (0 <= course <= 360):
        return None
    return {
        "mmsi": mmsi,
        "name": name,
...
async def ingest_ais(vessels: list[dict] | dict, x_ais_token: str | None = Header(default=None)) -> dict:
    ...
    Requires the AIS_INGEST_TOKEN shared secret (when configured) and rejects
    any fix with missing, malformed, or out-of-range fields so a bad or hostile
    provider payload can neither crash the live feed nor inject HTML into the
    dashboard for every connected client.
    """
    expected_token = os.getenv("AIS_INGEST_TOKEN")
    if expected_token and x_ais_token != expected_token:
        raise HTTPException(status_code=401, detail="Missing or invalid X-Ais-Token header")

    if isinstance(vessels, dict):
        vessels = [vessels]
    accepted = [fix for fix in (_clean_fix(vessel) for vessel in vessels) if fix is not None]
    rejected = len(vessels) - len(accepted)
    ...
    return {"accepted": len(accepted), "rejected": rejected, "connected_clients": len(live_manager.connections)}
